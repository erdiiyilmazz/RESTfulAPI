...
import com.erdi.microservice.restful.rest.users.exceptions.InvalidPermissionDataException;
import com.erdi.microservice.restful.rest.users.exceptions.PermissionInUseException;
import com.erdi.microservice.restful.rest.users.exceptions.PermissionNotFoundException;
...
public class PermissionService {
    ...
    public Permission getPermissionByKey(String key) {
        ...
        throw new PermissionNotFoundException(String.format("Please enter valid permission key, permission not found for permission key = %s", key));
    }
    ...
    public Permission createPermission(PermissionDTO permissionDTO) {
        ...
        if (permissionOpt.isPresent()) {
            throw new PermissionNotFoundException(String.format("Permission %s already exists with the same key with Id = %s",
        ...
        if (permissionByKeyOpt.isPresent()) {
            ...
            if (!permission1.getId().equals(permission.getId())) {
                throw new InvalidPermissionDataException(String.format("Exists already a permission with the key %s." +
                        " Please use another key", permissionKey));
            }
        }
    ...
    public void deletePermissionByKey(String permissionKey) {
        ...
        if (rowsFound > 0) {
            ...
            throw new PermissionInUseException(String.format("The permission with key %s is in use (%s configuration rows)",
